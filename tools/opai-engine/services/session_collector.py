"""OPAI Engine — Claude Session Collector.
...
_LIVE_CACHE_TTL = 4  # seconds — slightly under 5s poll interval


def get_live_usage() -> dict:
    """Lightweight live usage data optimized for frequent polling.

    Reads stats-cache.json for aggregate data and scans today's JSONL files
    across ALL project dirs for real-time today stats.
    """
    ...
    # Daily trend from stats-cache (last 7 days)
    daily_activity = stats.get("dailyActivity", [])[-7:]
    daily_tokens = stats.get("dailyModelTokens", [])[-7:]

    # Scan today's sessions across ALL project dirs
    today_input = 0
    today_output = 0
    today_cache_read = 0
        ...
        for project_dir in PROJECTS_DIR.iterdir():
            ...
            for jsonl_file in project_dir.glob("*.jsonl"):
                try:
                    ...
                    if fstat.st_mtime < today_midnight:
                        continue
    ...
    result = {
        ...
        "hourCounts": stats.get("hourCounts", {}),
    }
