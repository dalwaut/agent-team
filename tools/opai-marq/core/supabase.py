"""Marq — Supabase REST API helpers (async httpx)."""
...
async def _sb_post(path, data, token=None):
    ...
    async with httpx.AsyncClient(timeout=15) as c:
        ...
        if not r.is_success:
            import logging
            logging.getLogger("marq.supabase").error(
                "POST %s → %d: %s | payload keys: %s",
                path, r.status_code, r.text[:300], list(data.keys()) if isinstance(data, dict) else type(data),
            )
        r.raise_for_status()
        return r.json()
