...
const WORK_KEYWORDS = /\b(task|deadline|status|update|blocker|sprint|overdue|milestone|progress|assign|priority|due|blocked|standup|backlog|release|deploy|bug|issue|ticket|done|complete|in.progress|review|merge|ship|estimate|roadmap|goal|action.item|follow.up|sync)\b/i;
...
/**
 * Classify a message into one of 5 tiers.
...
function classifyMessage(text, ctx) {
  ...
  // Tier 4: Short ambiguous question
  const words = trimmed.split(/\s+/).length;
  if (words < 4 && trimmed.includes('?') && !WORK_KEYWORDS.test(trimmed)) {
    return { tier: 'unsure' };
  }
...
function getProcessingMessage(elapsed) {
  ...
  return `*Almost there...* (${formatElapsed(elapsed)})`;
}

// --- Claude Fire-and-Forget (coordinator persona) ---

async function processAssistantClaude(bot, { chatId, threadId, ackMessageId, scopeKey, username, content, workspaceName, state, contextBlock, useResume, sessionId }) {
  ...
  const prompt = buildCoordinatorPrompt(username, contextBlock, content, workspaceName);
  ...
  try {
    ...
    const elapsed = Math.round((Date.now() - startTime) / 1000);
    logAudit('assistant-claude', 'completed', `[${state}] Coordinator replied (${elapsed}s)`, { username, chatId, state });
    ...
    if (result.text && result.text.trim()) {
      ...
      console.log(`[TG] [ASST] Replied (${trimmed.length} chars, ${elapsed}s, state: ${state})`);
    } else {
    ...
    console.error(`[TG] [ASST] Error:`, err.message);
...
async function handleAssistantMessage(bot, ctx, opts) {
  ...
  console.log(`[TG] [ASST] ${username}: "${content.substring(0, 80)}" -> ${tier}${subtype ? ':' + subtype : ''}`);
  ...
  if (tier === 'op' || tier === 'work') {
    ...
    if (intent) {
      console.log(`[TG] [ASST] [FAST] "${cleanContent.substring(0, 60)}" -> ${intent.intent}`);
      try {
    ...
    // Fall through to Claude with coordinator prompt
    const { contextBlock, useResume, sessionId, state } = getContextStrategy(sharedKey);
    ...
    processAssistantClaude(bot, {
      ...
      workspaceName, state, contextBlock, useResume, sessionId,
  ...
  if (tier === 'unsure') {
    ...
    // Otherwise silent — message already recorded to buffer
    return;
  }
