...
def delegate_task(task_id: str) -> dict:
    ...
    task["updatedAt"] = datetime.now(tz.utc).isoformat()

    # Auto-route: classify + assign agentConfig so executor can pick it up
    try:
        wc_script = config.TOOLS_DIR / "work-companion" / "index.js"
        text = task["title"] + " " + (task.get("description") or "")
        result = subprocess.run(
            ["node", "-e", f"""
                const wc = require('{wc_script}');
                const c = wc.classifyTask({json.dumps(text)});
                const r = wc.routeTask(c);
                console.log(JSON.stringify({{type: c.type, squads: r.squads, agents: r.agents, mode: r.mode}}));
            """],
        ...
        if result.returncode == 0:
            ...
            task["routing"]["mode"] = "execute"  # Delegated = ready to run

            # Set agentConfig from routing result (executor requires this)
            ...
            task["routing"] = task.get("routing") or {"type": "unknown", "squads": ["review"], "mode": "execute"}
    except Exception:
        task["routing"] = task.get("routing") or {"type": "unknown", "squads": ["review"], "mode": "execute"}
