"""OPAI Monitor — Updater Agent.

Watches for changes to the OPAI system and generates categorized entries:
  - "update"  — Actionable: needs dashboard/config attention (new tool needs
                service entry, orphan prompt with no agent, etc.)
  - "notice"  — Informational: detected change, nothing to do (new agent
                already in team.json, report category auto-browsable, etc.)

Entries can be: pending, archived, tasked.
Archived entries are never re-suggested.

On first scan, the system records the baseline and generates NO entries.
Only subsequent scans that detect actual CHANGES produce entries.
"""
...
class UpdaterAgent:
    ...
    # Each suggestion gets a "kind": "update" or "notice"

    def _generate_suggestion(self, change: dict, context: dict) -> dict | None:
        """Generate a categorized suggestion from a change.

        context contains current scan results for cross-referencing
        (e.g. checking if a prompt file has a matching agent).
        """
        cat = change["category"]
        key = change["key"]
        ...
        if action == "added":
            if cat == "tools":
                # Is it already registered as a systemd service?
                ...
                if key.startswith("prompt_"):
                    ...
                    # Is there a matching agent in team.json?
                    if agent_name in agents:
                        return None  # Redundant — agent already tracked
                    else:
                ...
                return None  # Non-prompt scripts are not interesting
    ...
    async def scan(self):
        ...
        # On first scan: record baseline, generate nothing
        # On subsequent scans: generate entries for actual changes
        if not is_first:
            context = {"agents": agents, "services": services}
            existing_ids = {s["id"] for s in self.suggestions.get("suggestions", [])}
            # Also skip IDs that were archived (prevent re-suggestion)
            archived_ids = {s["id"] for s in self.suggestions.get("suggestions", [])
