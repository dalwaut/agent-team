"""OPAI Task Control Panel — Backend operations."""
...
# Default trusted senders — overridden by orchestrator.json trusted_senders list
...
def should_bypass_approval(task: dict) -> bool:
    """Return True if this task should skip the human approval gate.

    Bypass rules (task enters as 'scheduled', routing.mode='execute'):
    - source == "discord"     → admin-only channel, always bypass
    - source == "feedback"    → system-generated, always bypass
    - source == "email" and sender in trusted_senders → bypass
    Tasks from all other sources enter as 'pending' and require approval.
    """
    source = task.get("source", "")
    if source in ("discord", "feedback"):
        return True
    if source == "email":
        sender = (task.get("sourceRef") or {}).get("sender", "").lower().strip()
        try:
            if config.ORCHESTRATOR_JSON.is_file():
                data = json.loads(config.ORCHESTRATOR_JSON.read_text())
                trusted = {s.lower() for s in data.get("task_processor", {}).get("trusted_senders", [])}
            else:
                trusted = _DEFAULT_TRUSTED_SENDERS
...
def _determine_origin(task: dict) -> str:
    ...
    if task.get("bypassReason") == "trusted-email":
        return "trusted-email-bypass"
    if mode == "queued":
        return "hitl"
...
def parse_feedback_files() -> list[dict]:
    ...
    for fp in sorted(FEEDBACK_DIR.glob("Feedback-*.md")):
        ...
        for severity in ("HIGH", "MEDIUM", "LOW"):
            ...
            line_re = re.compile(
                r'^- (?P<struck>~~)?\*\*\[(?P<cat>[^\]]+)\]\*\* (?P<desc>.+?) _\((?P<id>[^,]+), (?P<ts>[^)]+)\)_'
                r'(?:~~ \*\*IMPLEMENTED\*\*.*)?$',
                re.MULTILINE,
            )
