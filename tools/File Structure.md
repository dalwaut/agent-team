...
- **Step 2: Per-Project OPAI Structure** – Projects under Projects/ use a standardized diamond layout for isolated work. Agents must not break isolation—e.g., read from Research, write to Notes/Logs, execute via Tasks.
...
- **Projects/**
  Description: Core workspace for OPAI project structure. Houses projects and ensures agent compatibility.
  Notes for Agents: Interact via defined paths—e.g., read/write to subfolders without altering structure. Spawn new projects here. Treat each as self-contained—e.g., Project Manager Agent oversees all.
...
- **tools/**
  Description: Helper scripts/wrappers, web services (opai-monitor, opai-portal, opai-files, etc.).
  Notes for Agents: Execute tools from here. Add new ones as needed.

  - **opai-files/**
    Description: Sandboxed file manager web service (port 8086). Users browse their sandbox; admins browse `/workspace/synced/opai`. FastAPI backend + vanilla JS SPA.
    Notes for Agents: Do not modify — this is a running service. Files are served via Caddy at `/files/`.
