"""HELM — Credential Vault (Fernet encryption for per-business secrets)."""
...
def list_credentials(business_id: str) -> list[str]:
    """List all vault keys for a business."""
    dir_path = config.VAULT_DIR / business_id
    if not dir_path.exists():
        return []
    return [
        f"imp/{business_id}/{p.stem.replace('.json', '')}"
