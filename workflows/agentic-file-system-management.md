...
| Empty projects | Projects/ | WE Sticker, WE WebApps, image-optimizer |
...
```
  ...
  notes/          Library/        Projects/       Clients/
...
| Archive | Empty projects: WE Sticker, WE WebApps, image-optimizer | Move to `Projects/_archived/` |
...
1. **Structure Compliance** — Scan every project in `Projects/` and `Clients/`. Report which tier (A/B/C) each project is, whether it has `PROJECT.md`, and what's missing from its expected structure.
...
OPAI (Master Workspace)
  ...
  |
  |-- Projects/              # 30+ projects (Tier A or Tier B)
