...
runners=("run_squad.sh" "run_agents.sh" "run_agents_seq.sh" "run_auto.sh" "preflight.sh" "familiarize.sh" "process_queue.sh" "onboard_project.sh" "migrate.sh")
...
if ! $DRY_RUN; then
    if [[ -f "$OPAI_ROOT/scripts/preflight.sh" ]]; then
        bash "$OPAI_ROOT/scripts/preflight.sh" || echo "  Preflight had warnings."
    else
        echo "  preflight.sh not found — skipping."
    fi
else
