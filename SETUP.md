...
# Ctrl+C after confirming "Connected as Orchestrator-Bot" message
# Then enable the systemd service for persistent running:
# systemctl --user enable --now opai-discord-bot
...
The Discord bot uses a **user-level** systemd service (no sudo required). The service file is already installed at `~/.config/systemd/user/opai-discord-bot.service`.

### Discord Bot Service

The bot runs via `start-bot.sh` which handles:
- Loading nvm (claude CLI is installed via nvm, not in system PATH)
- Stripping `CLAUDECODE` env var (prevents "nested session" error)
- PID lockfile for single-instance enforcement
- Killing orphaned instances before starting
...
systemctl --user enable opai-discord-bot
loginctl enable-linger $USER
```

**WARNING**: An old `opai-discord.service` exists but is **disabled**. Do NOT enable it — running both services causes duplicate Discord responses. Only use `opai-discord-bot`.

Repeat similar pattern for: `opai-email` (email checker on timer), `opai-monitor` (web dashboard).
